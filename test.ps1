...
# 1. PowerShell & cmd.exe
echo -e "\nChecking PowerShell..."
if check_command "powershell" "PowerShell" || check_command "pwsh" "PowerShell Core"; then
    powershell_version=$(powershell -Command '$PSVersionTable.PSVersion' 2>/dev/null || pwsh -Command '$PSVersionTable.PSVersion' 2>/dev/null)
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" || "$OSTYPE" == "win32" ]]; then
    if check_command "cmd" "cmd.exe"; then
        cmd_version=$(cmd /c ver 2>/dev/null)
    ...
    echo "[-] cmd.exe check skipped (not a Windows environment)."
fi
